        ...
        async with self.pool.acquire() as conn:
            ...
            for row in rows:
                try:
                    ...
                    await conn.execute(
                        "UPDATE index_queue SET status='error' WHERE id=$1",
                        row['id']
                    )
    ...
    async def _handle_upsert(self, conn, file_path: str):
        ...
        if existing:
            ...
            file_id = await conn.fetchval(
                ...
                file_path, file_hash, len(content)
            )
        # Chunk and embed
        ...
        for chunk, embedding in zip(chunks, embeddings):
            chunk_id = await conn.fetchval(
                ...
                file_id, chunk.chunk_index, chunk.text, chunk.start_line, chunk.end_line, chunk.token_count
            )
            await conn.execute(
