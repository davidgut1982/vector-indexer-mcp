"""Standalone test script for the file watcher daemon.

Tests configuration loading, event handling, and debouncing logic
without requiring heavy dependencies like sentence-transformers.
"""
...
# Import directly from modules (avoid __init__.py)
from daemon import config as config_module
from daemon import watcher as watcher_module
...
def test_config_loading():
    ...
    print(f"\nExclude patterns: {len(cfg.exclude_patterns)}")
    print(f"Include extensions: {len(cfg.include_extensions)}")
    print(f"Max file size: {cfg.max_file_size_mb} MB")
